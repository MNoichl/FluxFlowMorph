...
class FlowMorphSequenceSession:
    ...
    def __init__(self, runner: FlowMorphRunner) -> None:
    ...
    def fit_endpoint(
        ...
        *,
        ...
        result: EndpointOptimizationResult = optimize_endpoint(
            ...
            sigma_i=start.sigma_i,
            sigma_last=start.sigma_last,
            timestep_i=runner.schedule.timesteps[runner.config.flowmorph.start_timestep_index].to(self.device),
            predictor=predictor,
    ...
    def decode_frames(
        ...
        try:
            _move_module(vae, self.device)
            for frame in frames:
                decoded.append(runner._decode_one(frame.final_latent))
        finally:
            _move_module(vae, "cpu")
            _move_module(transformer, self.device)
            release_cuda_memory()
        ...
        *,
        ...
        try:
            ...
            for frame, destination in zip(frames, destinations, strict=True):
                ...
                image = runner._decode_one(frame.final_latent)
